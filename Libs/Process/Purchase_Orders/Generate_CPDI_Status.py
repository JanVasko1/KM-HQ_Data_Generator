...
def Generate_PO_CPDI_Messages(Settings: dict, 
                              ...
                              HQ_CPDI_Status_df: DataFrame) -> None:
    # --------------------------------------------- Defaults --------------------------------------------- #
    Can_Continue = True
                ...
                if CPDI_Status_Method == "Fixed":
                    ...
                    for CPDI_Status in CPDI_Status_List:
                        ...
                        if Export_NAV_Folder == True:
                            File_Manipulation.Export_NAV_Folders(NVR_FS_Connect_df=NVR_FS_Connect_df, HQ_Communication_Setup_df=HQ_Communication_Setup_df, Buy_from_Vendor_No=Buy_from_Vendor_No, File_Content=Current_line_json, HQ_File_Type_Path="HQ_CPDI_Import_Path", File_Name=PO_CPDI_File_Name, File_suffix="json")
                        else:
                            File_Manipulation.Export_Download_Folders(File_Content=Current_line_json, File_Name=PO_CPDI_File_Name, File_suffix="json")
