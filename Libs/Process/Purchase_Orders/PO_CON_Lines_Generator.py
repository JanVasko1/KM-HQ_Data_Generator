...
def Generate_PO_CON_Lines(Settings: dict, 
    ...
    Lines_df["Exported_Line_No"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="Exported_Line_No", Compare_Column_df1=["buyer_aid"], Compare_Column_df2=["Item_No"], Search_df=HQ_Item_Tr_Reg_Filtered, Search_Column="Exported_Line_No"), axis=1)
    Lines_df["quantity"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="quantity", Compare_Column_df1=["buyer_aid"], Compare_Column_df2=["Item_No"], Search_df=HQ_Item_Tr_Reg_Filtered, Search_Column="Quantity"), axis=1)
    Lines_df["ordered_quantity"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="ordered_quantity", Compare_Column_df1=["buyer_aid"], Compare_Column_df2=["Item_No"], Search_df=HQ_Item_Tr_Reg_Filtered, Search_Column="Quantity"), axis=1)
    Lines_df["item_category"] = "YN01"
    Lines_df["discontinued"] = False
    Lines_df["set"] = False
    Lines_df["bom"] = False
    Lines_df["bom_with_delivery_group"] = False
    Lines_df["cancelled"] = False
    ...
    if Can_Continue == True:
        if Price_Method == "Price List":
            Lines_df["price_amount"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="price_amount", Compare_Column_df1=["buyer_aid"], Compare_Column_df2=["Asset_No"], Search_df=Items_Price_List_Detail_df, Search_Column="DirectUnitCost"), axis=1)
        elif Price_Method == "Purchase Line":
            Lines_df["price_amount"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="price_amount", Compare_Column_df1=["buyer_aid"], Compare_Column_df2=["No"], Search_df=Purchase_Lines_df_Filtered, Search_Column="Direct_Unit_Cost"), axis=1)
    ...
    if Can_Continue == True:
        if UoM_Method == "Fixed":
            ...
            Lines_df["PO_UoM"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="PO_UoM", Compare_Column_df1=["buyer_aid"], Compare_Column_df2=["No"], Search_df=Purchase_Lines_df_Filtered, Search_Column="Unit_of_Measure_Code"), axis=1)
            Lines_df["order_unit"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="order_unit", Compare_Column_df1=["PO_UoM"], Compare_Column_df2=["Code"], Search_df=UoM_df, Search_Column="International_Standard_Code"), axis=1)
            Lines_df.drop(labels=["PO_UoM"], inplace=True, axis=1)
        elif UoM_Method == "HQ Item Transport Export":
            Lines_df["PO_UoM"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="PO_UoM", Compare_Column_df1=["buyer_aid", "Exported_Line_No"], Compare_Column_df2=["Item_No", "Exported_Line_No"], Search_df=HQ_Item_Tr_Reg_Filtered, Search_Column="Unit_of_Measure"), axis=1)
            Lines_df["order_unit"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="order_unit", Compare_Column_df1=["PO_UoM"], Compare_Column_df2=["Code"], Search_df=UoM_df, Search_Column="International_Standard_Code"), axis=1)
            Lines_df.drop(labels=["PO_UoM"], inplace=True, axis=1)
            ...
            if Can_Continue == True:
                if Free_Method == "Fixed":
                    ...
                    if Face_Sheet_Number != "":
                        ...
                        # Add to Lines_df
                        Free_Face_Sheet_dict = dict(zip(Lines_df_Columns, Free_Face_Sheet_Line_list))
                        Insert_Index += 1
                        Lines_df = Pandas_Functions.Dataframe_Insert_Row_at_position(Insert_DataFrame=Lines_df, Insert_At_index=Insert_Index, New_Row=Free_Face_Sheet_dict)
                    else:
                        pass
                    ...
                    def Select_Free(Frame_Body: CTkFrame, Lines_No: int, Insert_Index: int, Lines_df: DataFrame):
                        for i in range(2, 5):                           
                    ...
                    Button_Frame = Elements_Groups.Get_Widget_Button_row(Configuration=Configuration, Frame=Frame_Body, Field_Frame_Type="Single_Column" , Buttons_count=1, Button_Size="Small") 
                    Button_Confirm_Var = Button_Frame.children["!ctkframe"].children["!ctkbutton"]
                    Button_Confirm_Var.configure(text="Confirm", command = lambda: Select_Free(Frame_Body=Frame_Body, Lines_No=Lines_No, Insert_Index=Insert_Index, Lines_df=Lines_df))
    ...
    Lines_df["description_long"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="description_long", Compare_Column_df1=["supplier_aid"], Compare_Column_df2=["No"], Search_df=Items_df, Search_Column="Description"), axis=1)
    ...
    if Can_Continue == True:
        if Line_Flag_Label_Enabled == True:
            # Find Machines
            Lines_df["Material_Group_help"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="Material_Group_help", Compare_Column_df1=["supplier_aid"], Compare_Column_df2=["No"], Search_df=Items_df, Search_Column="Material_Group_NUS"), axis=1)
            mask_Machines = Lines_df["Material_Group_help"] == "0100"
            Machines_df = Lines_df[mask_Machines]  
            Machines_df = Machines_df.sort_index(ascending=False)
            Lines_df.drop(labels=["Material_Group_help"], inplace=True, axis=1)
            ...
            if Machines_df.empty:
                ...
                for machine_row in Machines_df.iterrows():
                    ...
                    # Add to Lines_df
                    Label_dict = dict(zip(Lines_df_Columns, Label_Line_list))
                    Lines_df = Pandas_Functions.Dataframe_Insert_Row_at_position(Insert_DataFrame=Lines_df, Insert_At_index=Machine_Index, New_Row=Label_dict)
        else:
            pass
    else:
        pass
    ...
    if Can_Continue == True:
        if Line_Flag_Item_EOL_Finished == True:
            Lines_df["Distribution_Status_NUS"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="Distribution_Status_NUS", Compare_Column_df1=["supplier_aid"], Compare_Column_df2=["No"], Search_df=Items_df, Search_Column="Distribution_Status_NUS"), axis=1)
            Lines_df["Blocked_for_Purchase"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="Blocked_for_Purchase", Compare_Column_df1=["Distribution_Status_NUS"], Compare_Column_df2=["Distribution_Status"], Search_df=Items_Distr_Status_df, Search_Column="Blocked_for_Purchase"), axis=1)
            conditions = [(Lines_df["Blocked_for_Purchase"] == True)]
            Lines_df = Pandas_Functions.Dataframe_Set_Value_on_Condition(Set_df=Lines_df, conditions=conditions, Set_Column="item_category", Set_Value="TAPA")
            Lines_df = Pandas_Functions.Dataframe_Set_Value_on_Condition(Set_df=Lines_df, conditions=conditions, Set_Column="discontinued", Set_Value=True)
            Lines_df.drop(labels=["Distribution_Status_NUS", "Blocked_for_Purchase"], inplace=True, axis=1)
        else:
            pass
    else:
        pass
    ...
    if Can_Continue == True:
        if Line_Flag_Always_Substitute == True:
            Lines_df["Substitute_No"] = ""
            Lines_df["Substitute_No"] = Lines_df.apply(lambda row: Pandas_Functions.Dataframe_Apply_Value_from_df2(row=row, Fill_Column="Substitute_No", Compare_Column_df1=["supplier_aid"], Compare_Column_df2=["No"], Search_df=Items_Substitutions_df, Search_Column="Substitute_No"), axis=1)
            Lines_df.loc[Lines_df["Substitute_No"] != "", "supplier_aid"] = Lines_df["Substitute_No"]
            Lines_df.loc[Lines_df["Substitute_No"] != "", "discontinued"] = False
            Lines_df.drop(labels=["Substitute_No"], inplace=True, axis=1)
        else:
            pass
    else:
        pass
    ...
    if Can_Continue == True:
        ...
        if Line_Flags_Enabled == True:
            if Line_Flags_Method == "Random Cancel":
                index_list = Lines_df.index.to_list()
                random_index = random.choice(seq=index_list)
                Lines_df.at[random_index, "cancelled"] = True
            elif Line_Flags_Method == "Random Finished":
                index_list = Lines_df.index.to_list()
                random_index = random.choice(seq=index_list)
                Lines_df.at[random_index, "item_category"] = "TAPA"
                Lines_df.at[random_index, "discontinued"] = True
                Lines_df.at[random_index, "cancelled"] = "No longer available"

                # TODO --> when Machine marked --> free of charge Items must be deleted according to Exported_Line_No
            elif Line_Flags_Method == "Prompt":
                Lines_No = len(Lines_df)
                def Select_Flags(Frame_Body: CTkFrame, Lines_No: int, Lines_df: DataFrame):
                    Lines_df_index = 0
                    for i in range(2, Lines_No):    
                        ...
                        if Use_Substitution == True:
                            ...
                            if Sub_Item_No != "":
                                Lines_df.at[Lines_df_index, "supplier_aid"] = Sub_Item_No
                            else:
                                pass
                        else:
                            pass
                        ...
                        if Use_Cancel == True:
                            Lines_df.at[Lines_df_index, "cancelled"] = True
                        else:
                            pass

                        Use_Finished = Finished_CTkEntry.get()
                        if Use_Finished == True:
                            Lines_df.at[Lines_df_index, "discontinued"] = True
                        else:
                            pass
                ...
                Button_Frame = Elements_Groups.Get_Widget_Button_row(Configuration=Configuration, Frame=Frame_Body, Field_Frame_Type="Single_Column" , Buttons_count=1, Button_Size="Small") 
                Button_Confirm_Var = Button_Frame.children["!ctkframe"].children["!ctkbutton"]
                Button_Confirm_Var.configure(text="Confirm", command = lambda: Select_Flags(Frame_Body=Frame_Body, Lines_No=Lines_No, Lines_df=Lines_df))
    ...
    # line_item_id
    Lines_df["Exported_Line_No"] = Lines_df["Exported_Line_No"].astype(int)
    Lines_df["line_item_id"] = Lines_df["Exported_Line_No"] // 100
